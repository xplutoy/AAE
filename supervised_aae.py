...
from sampler import gaussian, gaussian_mixture, swiss_roll, uniform
...
flags.DEFINE_string('z_dist', 'g', '')
flags.DEFINE_string('datasets', 'mnist', '')
flags.DEFINE_string('log_path', './results_supervised_aae/', '')
FLAGS = flags.FLAGS
...
class supervised_aae:
    def __init__(self):
        ...
        fake_zy = tf.concat([self.fake_z, self.y], axis=1)
        real_zy = tf.concat([self.real_z, self.y], axis=1)
        self.rec_x, _ = self.de(fake_zy, False)
        self.gen_x, _ = self.de(real_zy)
    ...
    def fit(self, sess, local_):
        for _ in range(local_):
            ...
            sess.run(self.a_optim, {self.x: x_real, self.y: one_hot_y})
            for _ in range(3):
        ...
        return sess.run([self.a_loss, self.g_loss, self.d_loss, self.fit_summary], {
            self.x: x_real, self.real_z: z_real_(FLAGS.bz * 5), self.y: one_hot_y})

    def gen(self, sess, bz):
        # y = np.array(list(range(FLAGS.y_dim)) * bz)
        y = np.hstack([np.ones(bz, dtype=np.int32) * i for i in range(FLAGS.y_dim)])
        z = np.tile(z_real_(bz), (FLAGS.y_dim, 1))
        one_hot_y = one_hot_(y, FLAGS.y_dim)
        return sess.run([self.gen_x, self.gen_summary], {self.real_z: z, self.y: one_hot_y})
...
def main(_):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=_gpu)) as sess:
        ...
        while True:
            ...
            images, gen_summary = _model.gen(sess, 10)
